...
服务端与客户端必须安装相同版本的本模组。涉及烟雾配置同步的版本使用严格网络协议，不支持新旧 JAR 混用。

## 本分支主要特性

### 烟雾系统

- 烟雾状态由服务端管理，客户端仅负责体积构建、粒子和可见性表现。
- 支持 `sphere` 球形和 `cylinder` 圆柱体预设，默认球形。
- 根据实际可通行空间构建烟雾体积，支持平滑动态扩散。
- 多颗烟雾重叠时对粒子格进行去重，并使用全局粒子预算。
- 烟外无法通过名牌看见烟内玩家；烟内近距离观察仍保留正常可见性。
- 本模组的爆炸与烟雾体积重叠时可驱散烟雾。
- 支持烟雾熄灭允许该行为的燃烧云。

### 燃烧云

- 支持沿地形向上爬 `1` 格、向下搜索 `3` 格，均可配置。
- 服务端缓存地形扩散区域，降低重复路径计算。
- 客户端丢弃燃烧云重叠区域，限制每 tick 粒子数量。
- 烟雾、效果检查与地形缓存刷新频率均可配置。

### C4

- C4、部署者、爆炸参数和粘附状态可持久化保存。
- 兼容旧地图中已有 C4、旧 C4 物品及旧引爆器 NBT。
- 每名玩家默认最多部署 `16` 个 C4；旧存量不会被强制删除。
- 默认 `30` 分钟静默清扫，区块卸载期间仍按世界时间计算。
- 跨维度查找已加载的目标，不会为遥控起爆强制加载区块。
- 连锁爆炸采用限流队列，默认每个服务器 tick 最多处理 `8` 个。
- 静止 C4 停止运动碰撞计算，并降低方块支撑检测和同步频率。
- C4 爆炸或清扫后，对应引爆器会同步显示“目标已失效”。

### 其他改进

- 修复检视音效重复叠加及切换物品后的残留音效。
- 投掷物具有强制清理期限，超时清除时不会触发爆炸或效果。
- 爆炸伤害、范围及破坏倍率具有安全上限，并修复爆心实体不受伤的问题。
- Lore、属性排版和颜色贴近 TACZ 风格，支持 TACZ `HideFlags`。
- Index 的 `tooltip` 字段支持翻译键和 HEX 颜色。
- 附魔战斗效果、铁砧附魔和附魔台附魔为三个独立开关。
- 兼容 LesRaisins 原有的近战武器内容包实现方式。
